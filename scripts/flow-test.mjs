...
/* ── §7.2 clicking a placed course opens the dialog ─────────────── */
{
  ...
  // a long course page has to be readable, not just present
  const body = dialog.locator('> div').last()
  const box = await body.evaluate((e) => ({ sh: e.scrollHeight, ch: e.clientHeight }))
  ok(`  … the body scrolls (${box.sh}px of content in ${box.ch}px)`, box.sh > box.ch + 2)
  await body.evaluate((e) => e.scrollTo(0, 99999))
  await page.waitForTimeout(250)
  ok('  … and actually moves', (await body.evaluate((e) => e.scrollTop)) > 50)
  ok('  … while the header stays put', await dialog.getByText('Placement').isVisible().catch(() => true))
  ok('  … with no scrape artefacts in the prose', !/-->|<!--/.test(t))
...
/* ── layout: sidebars collapse and the board takes the space ────── */
{
  ...
  // the only thing left of a closed panel is the tab that reopens it
  const tabs = page.locator('button[aria-label^="Show "]')
  ok('  … and each leaves a reopen tab on the window edge', (await tabs.count()) === 2)
  const boxes = await Promise.all((await tabs.all()).map((t) => t.boundingBox()))
  ok('  … flush against the edges', boxes.some((b) => b.x <= 1) && boxes.some((b) => b.x + b.width >= 1439))
  await tabs.first().click()
  await page.waitForTimeout(500)
  ok('  … and clicking a tab brings the panel back', (await boardWidth()) < full - 100)
  ok('no page errors', errs.length === 0, errs)
  await page.context().close()
}
